...
function toBase64Url(value: string): string { return btoa(value).replaceAll('+', '-').replaceAll('/', '_').replaceAll('=', ''); }
function fromBase64Url(value: string): string { const base64 = value.replaceAll('-', '+').replaceAll('_', '/'); return atob(base64 + '='.repeat((4 - (base64.length % 4)) % 4)); }
...
function encodeSelection(state: GameState): SharedSelection {
  switch (state.selection.kind) {
    ...
    case 'ids': {
      ...
      return { kind: 'ids', indices: state.dice.map((die, index) => ids.has(die.id) ? index : -1).filter((index) => index >= 0) };
    }
...
function decodeSelection(selection: SharedSelection | undefined, dice: Die[]): Selection {
  ...
  if (selection.kind === 'range' && Number.isInteger(selection.min) && Number.isInteger(selection.max)) return { kind: 'range', min: selection.min!, max: selection.max! };
...
function encodeState(state: GameState): SharedStateV3 { return { dice: state.dice, history: state.history, selection: encodeSelection(state) }; }
...
function makeDice(values: number[]): GameState['dice'] { return values.filter(isD6Value).map((value, index) => ({ id: `shared-${index}`, type: 'd6' as const, value, origin: 'roll' as const })); }
...
export function readSharedSession(): GameSession | null {
  ...
  try {
    ...
    for (const raw of payload.undo) { const decoded = decodeState(raw); if (!decoded) return null; undo.push(decoded); }
    const redo: GameState[] = [];
    for (const raw of payload.redo) { const decoded = decodeState(raw); if (!decoded) return null; redo.push(decoded); }
...
export async function shareSession(session: GameSession): Promise<'shared' | 'copied' | 'failed'> {
  ...
  if (typeof navigator.share === 'function') {
    ...
    catch (error) { if (error instanceof DOMException && error.name === 'AbortError') return 'failed'; }
  }
